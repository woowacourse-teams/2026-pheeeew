...
import com.pheeeew.legacy.data.local.device.DeviceTokenStorage
import com.pheeeew.legacy.data.remote.device.api.DeviceRegistrationApi
import com.pheeeew.legacy.data.remote.device.dto.DeviceAttestationDto
...
class DeviceRegistrationRepositoryImpl(
    ...
    override suspend fun refreshAccessToken(refreshToken: RefreshToken): AccessTokenInfo =
        try {
            ...
            AccessTokenInfo(
                accessToken =
                    AccessToken(response.accessToken).also {
    ...
    ): com.pheeeew.legacy.data.remote.device.dto.DeviceRegistrationResponseDto =
        try {
    ...
    private fun Throwable.toDeviceRegistrationException(): DeviceRegistrationException =
        when (this) {
            is DeviceRegistrationException -> {
                this
            }
            ...
            is ApiException.Unknown -> {
                when (code) {
                    "DEVICE-007" -> {
                        DeviceRegistrationException.RetryableServer(
                        ...
                        DeviceRegistrationException.Server(this)
                    }
                }
            }

            is ApiException.Network -> {
                DeviceRegistrationException.Network(this)
            }
