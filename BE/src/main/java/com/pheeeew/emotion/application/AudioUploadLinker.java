...
/**
 * 감정 등록·수정에 녹음 업로드를 연결하는 서버 내부 계약이다.
...
public interface AudioUploadLinker {
    ...
    /**
     * 업로드를 하나의 감정에 원자적으로 연결하고 영구 파일 참조값을 반환한다.
     * 수정에서도 해당 감정의 최초 등록 requestId를 전달한다. 같은 감정에 새 업로드를 연결할 수 있다.
     * 교체된 업로드도 다른 감정에 재사용하지 않으며 기존 객체를 여기서 삭제하지 않는다.
     *
     ...
     * 다른 요청에 연결됐다면 거부한다.
     *
     * <p>등록 호출자는 먼저 requestId로 기존 감정을 조회한다. 같은 기기의 재시도는 기존 결과를 반환하고,
     ...
     * @param requestId 감정 최초 등록 요청의 식별자. 수정 시에도 변경하지 않는다.
