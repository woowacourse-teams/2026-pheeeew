...
@Tag(name = "신고", description = "감정 신고 API")
public interface EmotionReportControllerApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - `emotionId`에는 신고할 감정의 ID를 담습니다. 1 이상의 값이어야 합니다.
                    - 존재하지 않는 감정이면 404를 반환합니다.
                    - 삭제된 감정도 신고할 수 있습니다. 신고 기록은 감정 삭제와 무관하게 남습니다.

                    ### 중복 신고

                    - 같은 기기는 같은 감정을 한 번만 신고할 수 있습니다.
                    - 이미 신고한 감정을 다시 신고하면 새로 저장하지 않고 최초 신고를 그대로 반환합니다.
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "200",
                    description = "이미 신고한 감정이라 최초 신고를 반환",
                    content = @Content(schema = @Schema(implementation = EmotionReportResponse.class))
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "감정 식별자나 신고 사유가 올바르지 않음",
                    content = @Content(
            ...
            @ApiResponse(
                    responseCode = "409",
                    description = "자기 감정을 신고함. 작성자를 모르는 감정은 이 판정을 하지 않는다",
                    content = @Content(
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = @ExampleObject(value = """
                                    {"code":"REPORT-002","message":"자기 감정은 신고할 수 없습니다."}
                                    """)
                    )
            ),
            @ApiResponse(
