...
@Tag(name = "감정 차단", description = "감정 하나를 내 지도와 목록에서 가리는 API")
public interface EmotionBlockControllerApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - 차단한 감정은 지도 조회와 목록 조회에서 사라집니다.
                    - 조회 요청에 `Authorization` 헤더를 함께 보내야 차단이 적용됩니다. 헤더가 없으면 차단 전과 같은 결과를 받습니다.
                    - 신고와는 독립적입니다. 차단해도 서버에 신고가 접수되지 않습니다.

                    ### 차단 대상

                    - `emotionId`에는 가릴 감정의 ID를 담습니다. 1 이상이어야 합니다.
                    - 존재하지 않는 감정이면 404를 반환합니다.
                    - 삭제된 감정도 차단할 수 있습니다.
                    - 자기가 등록한 감정도 차단할 수 있습니다.
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "200",
                    description = "이미 차단한 감정이라 최초 차단을 반환",
                    content = @Content(
                            mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = EmotionBlockResponse.class)
                    )
    ...
    @Operation(
            summary = "감정 차단 해제",
            description = """
                    - 경로의 `emotionId`에 대한 내 차단만 해제합니다. 다른 기기의 차단은 남습니다.
                    - 차단하지 않은 감정을 해제해도 204를 반환합니다. 같은 요청을 여러 번 보내도 결과가 같습니다.
                    - 그 감정의 작성자를 사용자 차단으로도 차단해 두었다면, 이 해제만으로는 다시 보이지 않습니다.
                    """,
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "감정 차단 해제 성공"),
            @ApiResponse(
                    responseCode = "400",
                    description = "감정 식별자 형식이 올바르지 않음",
                    content = @Content(
                            mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = @ExampleObject(value = """
